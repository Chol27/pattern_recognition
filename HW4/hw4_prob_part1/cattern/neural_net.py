...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # classifier loss.                                                          #
    #############################################################################
    regularize = ((W1**2).sum() + (W2**2).sum())

    softmax = np.exp(scores)
    softmax /= softmax.sum(axis=1, keepdims=True)

    label = softmax[np.arange(N), y]
    data_loss = -np.log(label).sum()/N

    loss = data_loss + reg*regularize/2
    #############################################################################
    #                              END OF TODO#2                                #
    #############################################################################
    ...
    #############################################################################
    # loss(softmax(x2))
    softmax[np.arange(N), y] = label - 1

    # W2x1 + b2 = x2
    grads['b2'] = softmax.sum(axis=0)/N
    grads['W2'] = np.matmul(x1_ReLU.T, softmax)/N + reg*W2

    grad_x1 = np.matmul(softmax, W2.T)
    grad_x1[x1 < 0] = 0
    grads['b1'] = grad_x1.sum(axis=0)/N
    grads['W1'] = np.matmul(X.T, grad_x1)/N + reg*W1
    #############################################################################
    #                              END OF TODO#3                                #
    #############################################################################
    return loss, grads
